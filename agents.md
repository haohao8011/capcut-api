...
│       ├── api.py                # ServerAPI 封装（httpx，cap_token bearer）
...
- [x] db_models.py：Client/Asset/Task/TaskLog/SetupCode 五张表
...
- [x] 服务端 dashboard 重设计（任务/客户端/用户 3 tab + 安装码列表）
...
- [x] test_clients.py（10 步）+ test_tasks.py（12 步）+ **test_wizard.py（9 步）** 端到端测试通过
- [x] 既有 32/32 鉴权测试未破坏
